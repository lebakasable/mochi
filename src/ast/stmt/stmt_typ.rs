...
impl Stmt {
    ...
    pub fn from_file_with_prelude(input_path: &String) -> Result<Vec<Self>, MocError> {
        let mut visited = HashSet::new();
        let prelude_path = String::from("std/prelude.moc");
            ...
            Some(annotations) => {
                for a in annotations {
                    ...
                    if types.contains_key(&tid) {
                        return Err(MocError::new(format!("Generic type {} cannot be used as it has already been defined elsewhere.", a.token.lexeme), a.token.loc));
                    }
                    out.push(tid);
                }
            }
    ...
    pub fn build_types_and_data<'a>(
        stmts: Vec<Self>,
    ) -> Result<(TypeMap, GlobalEnv<'a>, InitDataMap, UninitDataMap), MocError> {
