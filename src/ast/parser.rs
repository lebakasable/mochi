...
impl<'a> Parser<'a> {
    ...
    pub fn parse(mut self) -> Result<Vec<Stmt>, MocError> {
    ...
    fn unary(&mut self, op: Token) -> Result<Box<Expr>, MocError> {
    ...
    fn declaration(&mut self) -> Result<Vec<Stmt>, MocError> {
    ...
    fn requires(&mut self, start: Token) -> Result<Vec<Token>, MocError> {
        
        if let Err(t) = self.matches(TokenKind::Marker(Marker::Colon)) {
            return Err(MocError::new(
                format!(
        ...
        if reqs.is_empty() {
            return Err(MocError::new("Interface requirement list should not be empty.", start.loc).with_hint("Consider removing the `requires` list."))
        }

        Ok(reqs)
    }

    fn include(&mut self, start: Token) -> Result<Vec<Stmt>, MocError> {
    ...
    fn interface_impl(&mut self, start: Token) -> Result<Vec<Stmt>, MocError> {
    ...
    fn interface(&mut self, start: Token) -> Result<Vec<Stmt>, MocError> {
    ...
    fn interface_functions(&mut self, name: &Token) -> Result<Vec<Stmt>, MocError> {
        let mut fns = vec![];
    ...
    fn interface_associated_types(&mut self) -> Result<Vec<(TypeId, Token)>, MocError> {
    ...
    > {
        ...
        if name.lexeme.starts_with('+') {
            ...
            if name.lexeme != format!("+{}", impl_on.unwrap().lexeme) {
                return Err(MocError::new("Unexpected On Copy function name.", name.loc)
                    .with_hint(format!(
            ...
            if name.lexeme != format!("-{}", impl_on.unwrap().lexeme) {
                return Err(MocError::new("Unexpected On Drop function name.", name.loc)
                    .with_hint(format!(
        ...
        let outputs = match self.matches(TokenKind::Marker(Marker::Arrow)) {
            Ok(_) => {
                ...
                if outputs.is_empty() {
                    return Err(MocError::new("Expected a non-empty return list.", tok.loc));
                }
    ...
    fn args_list(&mut self, token: &Token) -> Result<Vec<UntypedArg>, MocError> {
        let mut args = vec![];
    ...
    fn unnamed_args_list(&mut self, token: &Token) -> Result<Vec<UntypedArg>, MocError> {
    ...
    fn maybe_mut_ident_list(&mut self) -> Result<Vec<IdentArg>, MocError> {
        let mut args = vec![];
    ...
    fn maybe_mut_ident(&mut self) -> Result<Option<IdentArg>, MocError> {
    ...
    fn parse_type(&mut self) -> Result<Option<Token>, MocError> {
    ...
    // Arg -> type_name (: IDENT)?
    fn parse_arg(&mut self) -> Result<Option<UntypedArg>, MocError> {
    ...
    fn block(&mut self, open: Token) -> Result<Box<Expr>, MocError> {
    ...
    fn expression(&mut self) -> Result<Box<Expr>, MocError> {
        let token = self.tokens.pop().unwrap();
        match &token.kind {
    ...
    fn match_expr(&mut self, token: Token) -> Result<Box<Expr>, MocError> {

        if let Err(e) = self.matches(TokenKind::Marker(Marker::LeftBrace)) {
            return Err(MocError::new(format!("Expected {} after {}, but found {} instead", Marker::LeftBrace, Keyword::Match, &e.kind), e.loc))
        }
        ...
        if let Err(e) = self.matches(TokenKind::Marker(Marker::RightBrace)) {
            return Err(MocError::new(format!("Expected {} after {} cases, but found {} instead", Marker::RightBrace, Keyword::Match, &e.kind), e.loc))
        }

        Ok(Box::new(Expr::Match(MatchExpr{ token, cases, else_case: finally})))
    }

    fn parse_tuple_expr(&mut self, token: Token) -> Result<Box<Expr>, MocError> {
        ...
        loop {
            match self.matches(TokenKind::Marker(Marker::RightBracket)) {
                ...
                Err(e) => {
                    if self.is_at_end() {
                        return Err(MocError::new(format!("Expected {}, but found end of file instead.", Marker::RightBracket), e.loc))
                    }
                    exprs.push(*self.expression()?);
                }
            }            
    ...
    fn cast(&mut self, cast_tok: Token) -> Result<Box<Expr>, MocError> {
        let open = match self.matches(TokenKind::Marker(Marker::LeftParen)) {
            Ok(t) => t,
            Err(t) => {
                return Err(MocError::new(
    ...
    fn enum_struct(&mut self, struct_token: Token) -> Result<Vec<Stmt>, MocError> {
        self.record(struct_token, RecordKind::EnumStruct)
    }

    // enum -> "enum" IDENT "{" IDENT+ "}"
    fn enumeration(&mut self, start: Token) -> Result<Vec<Stmt>, MocError> {
    ...
    // impls     -> "impl" ":" function+
    fn record(&mut self, start: Token, kind: RecordKind) -> Result<Vec<Stmt>, MocError> {
        ...
        if members.is_empty() {
            return Err(MocError::new(format!("{kind} members cannot be empty."), name.loc));
        }
    ...
    fn members(
        ...
        kind: &RecordKind,
    ) -> Result<Vec<UntypedMember>, MocError> {
        let mut members = vec![];
    ...
    fn member(
        ...
        typ: Option<&Token>,
        kind: &RecordKind,
    ) -> Result<Option<UntypedMember>, MocError> {
    ...
    fn function_list(&mut self, impl_on: Option<&Token>) -> Result<Vec<Stmt>, MocError> {
        let mut fns = vec![];
        loop {
            match (
    ...
    fn impl_section(&mut self, impl_on: &Token) -> Result<Option<Vec<Stmt>>, MocError> {
    ...
    fn if_block(&mut self, token: Token) -> Result<Box<Expr>, MocError> {
    ...
    fn else_if_condition(&mut self) -> Result<Vec<Expr>, MocError> {
        let mut cond = vec![];
    ...
    fn as_block(&mut self, token: Token) -> Result<Box<Expr>, MocError> {
        if let Err(t) = self.matches(TokenKind::Marker(Marker::LeftBracket)) {
            return Err(MocError::new(
                format!(
                    "Expected {} after {}, but found {} instead.",
    ...
    fn var(&mut self, token: Token) -> Result<ExprVar, MocError> {
    ...
    fn parse_while(&mut self, token: Token) -> Result<Box<Expr>, MocError> {
        let mut cond = vec![];
        while self.matches(TokenKind::Keyword(Keyword::Do)).is_err() {
            cond.push(*self.expression().map_err(|_| MocError::new(
    ...
    fn size_of(&mut self, token: Token) -> Result<Box<Expr>, MocError> {
        let open = match self.matches(TokenKind::Marker(Marker::LeftParen)) {
            Ok(t) => t,
            Err(t) => {
                return Err(MocError::new(
