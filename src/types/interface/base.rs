...
impl InterfaceBaseType {
    ...
    pub fn associated_type_id(&self, token: &Token, typ: &TypeId) -> Result<TypeId, MocError> {
    ...
    pub fn type_index(&self, token: &Token) -> Result<usize, MocError> {
    ...
    ) -> Result<String, MocError> {
        let mut err = MocError::new_type_err(
            format!(
...
pub fn check_requirements<'a>(
    ...
    types: &mut TypeMap,
    map: &HashMap<TypeId, TypeId>,
) -> Result<(), (Option<&'a Token>, MocError)> {
...
pub fn validate_requirements(requirements: &Vec<Token>, types: &TypeMap) -> Result<(), MocError> {
